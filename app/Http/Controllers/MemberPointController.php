...
class MemberPointController extends Controller
{
    ...
    {
        $query = DB::connection('mysql_secondary')->table('costumers as c')
                    ->select('c.id', 'c.name', 'c.email', 'c.telepon',
                            DB::raw('MAX(p.cabang_id) as cabang_id'),
                            DB::raw("MAX(p.updated_at) as updated_at"),
                            DB::raw("SUM(CASE WHEN p.type = 1 THEN p.point ELSE 0 END) - SUM(CASE WHEN p.type IN (0, 2) THEN p.point ELSE 0 END) AS saldo_point"))
                    ->leftJoin('point as p', 'p.costumer_id', '=', 'c.id')
                    ->groupBy('c.id');
        ...
        // }
        $d = $query->orderBy('updated_at', 'desc')->paginate(10)->withQueryString();

        $customerIds = $d->pluck('id');

        $dataPoint = DB::connection('mysql_secondary')->table('point')
            ->whereIn('costumer_id', $customerIds)
            ->orderBy('created_at', 'desc')
            ->get()
            ->groupBy('costumer_id');

        $mapped = $d->getCollection()->map(function ($c) use ($dataPoint) {
            return [
                'id' => $c->id,
                'name' => $c->name,
                'email' => $c->email,
                'telepon' => $c->telepon,
                'saldo_point' => $c->saldo_point,
                'cabang_id' => $c->cabang_id,
                'updated_at' => $c->updated_at,
                'points' => $dataPoint->get($c->id)?->values() ?? [],
            ];
        });

        $d->setCollection($mapped);
        ...
        return Inertia::render('MemberPoint/Index', [
            'd' => $d,
            'jabatans' => $jabatans,
            'divisis' => $divisis,
