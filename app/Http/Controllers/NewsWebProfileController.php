...
class NewsWebProfileController extends Controller
{
    public function index(Request $request)
    {
        $query = DB::connection('mysql_secondary')->table('webprofile_news as ns');

        if ($request->filled('search')) {
            $search = $request->search;
            $query->where(function($q) use ($search) {
                $q->where('ns.title', 'like', "%$search%");
            });
        }
        ...
        // }
        $d = $query->orderBy('ns.id', 'desc')->paginate(10)->withQueryString();
        ...
        return Inertia::render('NewsWebProfile/Index', [
            'd' => $d,
            'jabatans' => $jabatans,
            'divisis' => $divisis,
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'thumbnail' => 'nullable|file|mimes:jpg,jpeg,png|max:1024',
        ...
        if ($request->hasFile('thumbnail')) {
            $request->file('thumbnail')->move(public_path('assets/justusku_web_profile/images/'), $filenamethumbnail);
            $thumbnailPath = 'assets/justusku_web_profile/images/' . $filenamethumbnail;
        }
        ...
        dd('test');
        DB::table('webprofile_news')->insert([
            'title' => $request->title,
    ...
    public function update(Request $request, $id)
    {
        ...
        $data = DB::connection('mysql_secondary')->table('webprofile_news')->where('id', $id)->first();
        ...
        if ($request->hasFile('thumbnail')) {
            // Hapus file lama
            if ($data->thumbnail && File::exists(public_path($data->thumbnail))) {
                File::delete(public_path($data->thumbnail));
            }
        ...
        dd($request);
        DB::table('webprofile_news')->where('id', $id)->update([
            'title' => $request->title,
            'thumbnail' => $thumbnailPath,
            'image' => $imagePath,
            'content' => $request->content,
