...
class DataNewsController extends Controller
{
    ...
    {
        $query = DB::connection('mysql_secondary')->table('news as ns');

        if ($request->filled('search')) {
            $search = $request->search;
            $query->where(function($q) use ($search) {
                $q->where('ns.judul', 'like', "%$search%");
            });
        }
    ...
    public function store(Request $request)
    {
        ...
        // Upload files
        $filenamePicture = time().'_'.$request->file('picture')->getClientOriginalName();

        $picturePath = null;

        if ($request->hasFile('picture')) {
            $request->file('picture')->move(public_path('assets/file_news'), $filenamePicture);
            $picturePath = 'assets/file_news/' . $filenamePicture;
        }
        dd('test');
    ...
    public function update(Request $request, $id)
    {
        ...
        $data = DB::connection('mysql_secondary')->table('news')->where('id', $id)->first();
        ...
        if ($request->hasFile('picture')) {
            // Hapus file lama
            if ($data->picture && File::exists(public_path($data->picture))) {
                File::delete(public_path($data->picture));
            }

            $filenameImage = time().'_'.$request->file('picture')->getClientOriginalName();
            $request->file('picture')->move(public_path('assets/file_news'), $filenameImage);
            $picturePath = 'assets/file_news/' . $filenameImage;
        }
        dd($request);
