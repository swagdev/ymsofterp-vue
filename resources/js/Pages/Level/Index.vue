...
import LevelFormModal from './LevelFormModal.vue';
...
const debouncedSearch = debounce(() => {
  router.get('/level-data', { search: search.value, status: showInactive.value ? 'inactive' : 'active' }, { preserveState: true, replace: true });
}, 100);

watch(showInactive, (val) => {
  router.get('/level-data', { search: search.value, status: val ? 'inactive' : 'active' }, { preserveState: true, replace: true });
});
    ...
    <div class="max-w-7xl w-full mx-auto py-8 px-2">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <i class="fa-solid fa-users text-blue-500"></i> Data Level
        </h1>
      ...
      <div class="bg-white rounded-2xl shadow-2xl overflow-x-auto transition-all">
        <table class="w-full min-w-full divide-y divide-gray-200">
          <thead class="bg-gradient-to-r from-blue-50 to-blue-100">
            <tr>
              <th class="px-6 py-3 text-left text-xs font-bold text-blue-700 uppercase tracking-wider rounded-tl-2xl">NAMA Level</th>
              <th class="px-6 py-3 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">NILAI Level</th>
              <th class="px-6 py-3 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">NILAI PH</th>
              <th class="px-6 py-3 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">NILAI DASAR POTONGAN BPJS</th>
              <th class="px-6 py-3 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">NILAI POINT GOLONGAN</th>
              <th class="px-6 py-3 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">QA REWARD</th>
              <th class="px-6 py-3 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">QA PUNISHMENT</th>
          ...
          <tbody>
            ...
            <tr v-for="(ds, key) in d.data" :key="ds.id" class="hover:bg-blue-50 transition shadow-sm">
              ...
              <td class="px-6 py-3">{{ ds.nilai_level }}</td>
              <td class="px-6 py-3">{{ ds.nilai_public_holiday }}</td>
              <td class="px-6 py-3">{{ ds.nilai_dasar_potongan_bpjs }}</td>
              <td class="px-6 py-3">{{ ds.nilai_point }}</td>
              <td class="px-6 py-3">{{ ds.qa_reward }}</td>
              <td class="px-6 py-3">{{ ds.qa_penalty }}</td>
              <td class="px-6 py-3">
                <button
