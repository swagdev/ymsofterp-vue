...
const props = defineProps({
  ...
  outlets: Object,
  roles: Object,
  newNik: Object, // untuk validasi nik baru
});

const fileInputIdCard = ref(null)
const fileInputFamilyCard = ref(null)
const fileInputLatestColorPhoto = ref(null)
...
const form = useForm({
    nik : props.customer?.nik || props.newNik,
...
function onSubmit() {
  ...
  form.upload_id_card = form.id_card_file || form.upload_id_card;
  form.upload_family_card = form.family_card_file || form.upload_family_card;
  form.upload_latest_color_photo = form.latest_color_photo_file || form.upload_latest_color_photo;
  console.log('form data', form.upload_latest_color_photo, form.latest_color_photo_file);
  if (isEdit.value) {
...
function getImageSrc(value) {
  if (typeof value === 'string') {
    // Jika blob atau absolute URL
    if (value.startsWith('blob:') || value.startsWith('http')) {
      return value
    }
    // Jika path dari Laravel (misal 'employee/foto.jpg')
...
//   if (file) {
//     form.upload_id_card = URL.createObjectURL(file)
...
function handleFileChange(event, type) {
  ...
  if (type === 'id_card') {
    form.upload_id_card = preview;
    form.id_card_file = file;
  }
  
  if (type === 'family_card') {
    form.upload_family_card = preview;
    form.family_card_file = file;
  }
  console.log('coba file ', type, preview, file);
  if (type === 'latest_color_photo') {
    form.upload_latest_color_photo = preview;
    form.latest_color_photo_file = file;
  }
}
...
function removeImage(type) {
  if (type === 'id_card') {
    ...
    form.id_card_file = null
    fileInputIdCard.value.value = ''
  }

  if (type === 'family_card') {
    form.upload_family_card = null
    form.family_card_file = null
    fileInputFamilyCard.value.value = ''
  }

  if (type === 'latest_color_photo') {
    form.upload_latest_color_photo = null
    form.latest_color_photo_file = null
    fileInputLatestColorPhoto.value.value = ''
  }
}
    ...
    <div class="max-w-5xl w-full mx-auto py-8 px-2">
      ...
      <form @submit.prevent="onSubmit" class="space-y-6" enctype="multipart/form-data">
          ...
          <div>
            ...
            <div v-if="form.upload_id_card" class="mt-2 relative">
              <img :src="getImageSrc(form.upload_id_card)" alt="Preview KTP" class="h-32 rounded-md border border-gray-200">
              <button 
                @click="(e) => removeImage('id_card')"
                type="button"
          ...
          <div>
            ...
            <div v-if="form.upload_family_card" class="mt-2 relative">
              <img :src="getImageSrc(form.upload_family_card)" alt="Preview KTP" class="h-32 rounded-md border border-gray-200">
              <button 
                @click="removeImage('family_card')"
                type="button"
                class="absolute top-0 right-0 bg-red-500 text-white rounded-full p-1 hover:bg-red-600"
              >
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div v-else class="mt-1">
              <input
                ref="fileInputFamilyCard"
                type="file"
                @change="(e) => handleFileChange(e, 'family_card')"
                accept="image/*"
                class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
              >
              <p class="mt-1 text-xs text-gray-500">Format: JPG/PNG (Maks. 1MB)</p>
          ...
          <div>
            ...
            <div v-if="form.upload_latest_color_photo" class="mt-2 relative">
              <img :src="getImageSrc(form.upload_latest_color_photo)" alt="Preview Photo Berwarna Terbaru" class="h-32 rounded-md border border-gray-200">
              <button 
                @click="removeImage('latest_color_photo')"
                type="button"
                class="absolute top-0 right-0 bg-red-500 text-white rounded-full p-1 hover:bg-red-600"
              >
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div v-else class="mt-1">
              <input
                ref="fileInputLatestColorPhoto"
                type="file"
                @change="(e) => handleFileChange(e, 'latest_color_photo')"
                accept="image/*"
                class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
              >
              <p class="mt-1 text-xs text-gray-500">Format: JPG/PNG (Maks. 1MB)</p>
